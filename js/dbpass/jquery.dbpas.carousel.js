...
(function ($, win, doc, undefined) {
    ...
    var Carousel = {
        init: function (options, elem) {
            ...
            }); //give all items uniform width for smooth movement
            self.$wrapper.css({
                'max-width': (self.maxOuterWidth * self.options.itemsVisible) + 'px'
            }); //adjust wrapper width to show X items
            self.$elem.css({
                'left': '-' + self.maxOuterWidth + 'px'
            }); //move list to the left of view by 1 item width
        ...
        navigation: function (direction) {
            ...
            self.$elem.not(':animated').animate({
                ...
                function () {
                    ...
                    self.$elem.css({
                        'left': '-' + self.maxOuterWidth + 'px'
                    }); //move list to the left/right of view by 1 item width
                }
            );
        },
